...
class Conv(object):
  ...
  def backward(dout, cache):
    ...
    dx = dx[:,:,pad:-pad,pad:-pad]  #去掉padding的像素点
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx, dw, db
...
class MaxPool(object):
  ...
  def backward(dout, cache):
    ...
    #max pooling 反向传播时只需要把梯度传给最大值：1.找到最大值indices 2.用dout赋值
    
    for n in range (N):
        for height in range(H_out):
            for width in range(W_out):
                local_x = x[n, :, height*stride:height*stride+pool_height, width*stride:width*stride+pool_width]
                shape_local_x = local_x.shape #保存每个池化小窗口的shape
                reshaped_local_x = local_x.reshape(C, -1) #为了方便查找最大值
                local_dx = torch.zeros_like(reshaped_local_x)
                values, indicies = reshaped_local_x.max(-1)#找最大值索引
                local_dx[range(C), indicies] = dout[n, :, height, width]
                dx[n, :, height*stride:height*stride+pool_height, width*stride:width*stride+pool_width] += local_dx.reshape(shape_local_x) 
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx
...
class ThreeLayerConvNet(object):
  ...
  def loss(self, X, y=None):
    ...
    scores = out_L
    '''
    scores_conv, cache_conv = Conv.forward(X, W1, b1, conv_param)
    scores_ReLU, cache_ReLU = ReLU.forward(scores_conv)
    scores_pool, cache_pool = MaxPool.forward(scores_ReLU, pool_param)
    
    #N = X.shape[0]
    #scores_pool = scores_pool.reshape(N, -1)
    scores_LR, cache_LR = Linear_ReLU.forward(scores_pool, W2, b2)
    scores, cache_L = Linear.forward(scores_LR, W3, b3)
    '''
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class DeepConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    #初始化权重
    depth_filters, H_out, W_out = input_dims#depth_filters:为了计算卷积核的深度，H_out,W_out:为了计算最后一层线形层参数的shape
    #初始化卷积层
    for i , num_filter in enumerate(num_filters):
        ...
        if self.batchnorm:
            self.params['gamma{}'.format(i)] = torch.ones(num_filter, device = device, dtype = dtype)#num_filters=特征图的深度=要初始化的gamma个数
            self.params['beta{}'.format(i)] = torch.zeros(num_filter, device = device, dtype = dtype)
        if i in max_pools:
            H_out = int(1 + (H_out - pool_param['pool_height']) / pool_param['stride'])
            W_out = int(1 + (W_out - pool_param['pool_width']) / pool_param['stride'])
        if weight_scale == 'kaiming':
            self.params['W{}'.format(i)] = kaiming_initializer(num_filter, depth_filters, K=filter_size, relu=True, device=device, dtype=dtype)
        else:
            self.params['W{}'.format(i)] = weight_scale*torch.randn(num_filter, depth_filters, filter_size, filter_size, device=device, dtype=dtype)
        self.params['b{}'.format(i)] = torch.zeros(num_filter, device=device, dtype=dtype)
        depth_filters = num_filter
    #初始化线性层
    i += 1
    if weight_scale == 'kaiming':
  ...
  def loss(self, X, y=None):
    ...
    cache = {} #dictionary 储存每一步forward的信息
    mid_out = X #forward中间输出，也是下一步的输入
    
    #卷积层的前向传播
    for i in range(self.num_layers-1):
        if self.batchnorm:
            if i in self.max_pools:
                mid_out, cache['{}'.format(i)] = Conv_BatchNorm_ReLU_Pool.forward(mid_out, self.params['W{}'.format(i)], self.params['b{}'.format(i)], self.params['gamma{}'.format(i)], self.params['beta{}'.format(i)], conv_param, self.bn_params[i], pool_param)
            else:
                mid_out, cache['{}'.format(i)] = Conv_BatchNorm_ReLU.forward(mid_out, self.params['W{}'.format(i)], self.params['b{}'.format(i)],
          self.params['gamma{}'.format(i)], self.params['beta{}'.format(i)], conv_param, self.bn_params[i])
        else:
            if i in self.max_pools:
                mid_out, cache['{}'.format(i)] = Conv_ReLU_Pool.forward(mid_out, self.params['W{}'.format(i)], self.params['b{}'.format(i)], conv_param, pool_param)
            else:
                mid_out, cache['{}'.format(i)] = Conv_ReLU.forward(mid_out, self.params['W{}'.format(i)], self.params['b{}'.format(i)], conv_param)
    #线形层前向传播
    i += 1
    scores, cache['{}'.format(i)] = Linear.forward(mid_out, self.params['W{}'.format(i)], self.params['b{}'.format(i)])
    ...
    #线性层反向传播
    mid_dout, dw, db = Linear.backward(dout, cache['{}'.format(i)])
    ...
    for i in range(i-1, -1, -1):
        if i in self.max_pools:
            if self.batchnorm:
                mid_dout, dw, db, dgamma1,dbetta1  = Conv_BatchNorm_ReLU_Pool.backward(mid_dout, cache['{}'.format(i)])
                grads['W{}'.format(i)] = dw + 2*self.params['W{}'.format(i)]*self.reg
                grads['b{}'.format(i)] = db
                grads['beta{}'.format(i)] = dbetta1
                grads['gamma{}'.format(i)] = dgamma1 + 2*self.params['gamma{}'.format(i)]*self.reg
            else:  
                mid_dout, dw, db  = Conv_ReLU_Pool.backward(mid_dout, cache['{}'.format(i)])
                grads['W{}'.format(i)] = dw + 2*self.params['W{}'.format(i)]*self.reg
                grads['b{}'.format(i)] = db
        else:
            if self.batchnorm:
                mid_dout, dw, db,dgamma1,dbetta1  = Conv_BatchNorm_ReLU.backward(mid_dout, cache['{}'.format(i)])
                grads['W{}'.format(i)] = dw + 2*self.params['W{}'.format(i)]*self.reg
                grads['b{}'.format(i)] = db
                grads['beta{}'.format(i)] = dbetta1
                grads['gamma{}'.format(i)] = dgamma1 + 2*self.params['gamma{}'.format(i)]*self.reg
            else:
                mid_dout, dw, db  = Conv_ReLU.backward(mid_dout, cache['{}'.format(i)])
                grads['W{}'.format(i)] = dw + 2*self.params['W{}'.format(i)]*self.reg
                grads['b{}'.format(i)] = db
...
def create_convolutional_solver_instance(data_dict, dtype, device):
  ...
  model = DeepConvNet(input_dims = input_dims, num_classes = 10, 
                      num_filters = [32, 64, 128], max_pools=[0,1,2], 
                      weight_scale='kaiming', reg = 1e-5, 
                      dtype = dtype, device = device)
  
  solver = Solver(model,data_dict,num_epochs = 100,
...
class BatchNorm(object):
  ...
  def backward(dout, cache):
    ...
    dvar = dsqrtvar*(0.5*1. / torch.sqrt(var+eps))
  ...
  def backward_alt(dout, cache):
    ...
    dx = (gamma*ivar/N) * (N*dout - xhat*dgamma*(1 - 1/N) - dbeta)  
...
class SpatialBatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    # Replace "pass" statement with your code
    N, C, H, W = x.shape
    reshape_x = x.permute(1,0,2,3).reshape(C, -1).t() #适合于N×D的
    reshape_x_norm, cache = BatchNorm.forward(reshape_x, gamma, beta, bn_param)
    out = reshape_x_norm.t().reshape(C, N, H, W).permute(1, 0, 2, 3)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
  ...
  def backward(dout, cache):
    ...
    # Replace "pass" statement with your code
    N, C, H, W = dout.shape
    reshape_dout = dout.permute(1, 0, 2, 3).reshape(C, -1).t()
    dx, dgamma, dbeta = BatchNorm.backward_alt(reshape_dout, cache)
