...
class Linear(object):
  ...
  def backward(dout, cache):
    ...
    dw = x.view(x.shape[0],-1).t().mm(dout)#dimension要对应上
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx, dw, db
...
class ReLU(object):
  ...
  def forward(x):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    out = x.clone() #为了不改变x，保留x的信息用作计算gradient
    out[out < 0] = 0
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for n, hidden_dim in enumerate(hidden_dims):
        i = n+1
        self.params['W{}'.format(i)] = weight_scale*torch.randn(Mid_dim, hidden_dim, dtype = dtype, device = device)
        self.params['b{}'.format(i)] = torch.zeros(hidden_dim, dtype = dtype, device = device)
        Mid_dim = hidden_dim
        
    i += 1
    self.params['W{}'.format(i)] = weight_scale*torch.randn(Mid_dim, num_classes, dtype = dtype, device = device)
  ...
  def loss(self, X, y=None):
    ...
    mid_out = X
    cache_dict = {}
    for n in range(self.num_layers-1): #最后一层没有ReLU
        i = n+1
        mid_out, cache_dict['cache_LR{}'.format(i)] = Linear_ReLU.forward(mid_out, self.params['W{}'.format(i)], self.params['b{}'.format(i)])
        if self.use_dropout:
            mid_out, cache_dict['cache_Dropout{}'.format(i)] = Dropout.forward(mid_out, self.dropout_param)
    i += 1
    mid_out, cache_dict['cache_L{}'.format(i)] = Linear.forward(mid_out, self.params['W{}'.format(i)], self.params['b{}'.format(i)])
    scores = mid_out
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    mid_dout, dw, db = Linear.backward(dout, cache_dict['cache_L{}'.format(i)])
    grads['W{}'.format(i)] = dw + 2*self.params['W{}'.format(i)]*self.reg
    grads['b{}'.format(i)] = db
    
    for n in range(self.num_layers-1)[::-1]:  #tips
        i = n+1
        if self.use_dropout:
            mid_dout = Dropout.backward(mid_dout, cache_dict['cache_Dropout{}'.format(i)])
        mid_dout, dw, db = Linear_ReLU.backward(mid_dout, cache_dict['cache_LR{}'.format(i)])
        grads['W{}'.format(i)] = dw + 2*self.params['W{}'.format(i)]*self.reg
        grads['b{}'.format(i)] = db
        loss += (self.params['W{}'.format(i)]*self.params['W{}'.format(i)]).sum()*self.reg
    ############################################################################
