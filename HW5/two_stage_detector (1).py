...
class ProposalModule(nn.Module):
  ...
  def forward(self, features, pos_anchor_coord=None, \
    ...
    if mode == 'eval':
      ...
      conf_scores = torch.cat((pos_anchors[:,:2], neg_anchors[:,:2]), 0)
      offsets = pos_anchors[:,2:]
      proposals = GenerateProposal(pos_anchors[:,2:].reshape(pos_anchor_coord.shape[0],1,1,1,4),offsets.reshape(pos_anchor_coord.shape[0],1,1,1,4),method = 'FasterRCNN')
...
class RPN(nn.Module):
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7, mode='RPN'):
    ...
    with torch.no_grad():
      # B, _, H, W = image.shape # (B, 3, H, W)
      features = self.feat_extractor(images) # (B, ?, H, W)
      ...
      # conf_scores, offsets: (B, A, 2, H', W') and (B, A, 4, H', W')
      conf_probs = torch.sigmoid(conf_scores[:, :, 0, :, :]).squeeze() # (B, A, H', W') # conf_probs
      ...
      for b in range(B):
        ...
        # print(conf_probs[b])
        # print(index)
        # print(conf_probs[b][index])
        # print(proposals[b][index].shape)
        masked = conf_probs[b][index].view(-1) # Pi
        # print(masked,'\n',masked.shape)
        s_proposal = proposals[b][index].view(-1, 4) 
        # print(s_proposal.shape)
        
        left = torchvision.ops.nms(s_proposal, masked, nms_thresh)
        # print(masked[left].unsqueeze(1).shape)
        # print(s_proposal[left].shape)
        # print(masked[left].shape)
        final_proposals.append(s_proposal[left]) #(*x4)
        final_conf_probs.append(masked[left].unsqueeze(1)) #(*)
        # print("______________")
        # print(final_proposals[0:3])
        # print(final_conf_probs[0].shape)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    if mode == 'RPN':
...
class TwoStageDetector(nn.Module):
  ...
  def forward(self, images, bboxes):
    ...
    # print(out.shape)
    rois = torch.zeros((proposals.shape[0],5),device=proposals.device, dtype=proposals.dtype)
    rois[:,0] = (pos_anchor_idx // anc_per_img)
    rois[:,1:5] = proposals
    proposals = torchvision.ops.roi_align(features,rois,self.output_size)
    # import pdb; pdb.set_trace()
    roi_pooled =torch.mean(proposals,(2,3),False)
