...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    scores_1 = scores - scores.max(dim = 1, keepdim = True).values
    scores_2 = scores_1.exp().sum(dim = 1, keepdim = True)
    log_probs = scores_1 - scores_2.log()
    probs = log_probs.exp()
    ...
    d_scores[torch.arange(N), y] -= 1 #1.标量对矩阵求导 
    d_scores /= N
    
    grads['b2'] = d_scores.sum(dim = 0)
    grads['W2'] = h1.t().mm(d_scores) + 2* W2 * reg #2.矩阵对矩阵求导
    
    d_h1 = d_scores.mm(W2.T)
    d_h1[h1 == 0] = 0 #for ReLU
...
def nn_predict(params, loss_func, X):
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  results,_ = nn_forward_pass(params, X) #scores
  _, y_pred =  results.max(dim = 1) #idx
  ###########################################################################
  #                              END OF YOUR CODE                           #
  ###########################################################################
