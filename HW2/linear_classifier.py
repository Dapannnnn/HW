...
def svm_loss_vectorized(W, X, y, reg):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  correct_class_y = torch.zeros_like(scores,dtype = torch.bool)
  correct_class_y[range(num_train),y] = True
  scores[scores > 0] = 1
  scores[scores < 0] = 0
  scores[range(num_train),y] = torch.sum(scores,dim = 1)*-1
  dW = scores.t().mm(X).t()
  dW = dW.div(num_train)
  #############################################################################
  #                             END OF YOUR CODE                              #
...
def svm_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [0.000001,0.0001, 0.001, 0.005,0.01,0.02]
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    scores = W.t().mv(X[i])
    scores = scores- scores.max()  #这一步意义何在？为了减少计算量吗？
    correct_class_score = scores[y[i]]
    loss += -correct_class_score + torch.log(torch.exp(scores).sum())
    #print(loss,-correct_class_score,torch.log(torch.exp(scores).sum()))
    for j in range(num_classes):
      if j == y[i]:
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  exp_scores_sum = exp_scores.sum(dim = 1) #dim = 1
  #print('exp_scores_sum:',exp_scores_sum)
...
def softmax_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [0.001,0.01,0.02,0.1]
  regularization_strengths = [0.0001,0.0001,0.001,0.01,0.1]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
