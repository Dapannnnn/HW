...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  H = (H - kernel_size_1 + 2*2)/stride + 1#for fully connected layer
  W = (W - kernel_size_1 + 2*2)/stride + 1
  ...
  H = (H - kernel_size_2 + 2*1)/stride + 1#for fully connected layer
  W = (W - kernel_size_2 + 2*1)/stride + 1
  H = int(H)
  W = int(W)
...
class ThreeLayerConvNet(nn.Module):
  ...
  def forward(self, x):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    scores_mid = F.relu(self.cn2(F.relu(self.cn1(x))))
    scores_mid = flatten(scores_mid)
    scores = self.fc1(scores_mid)
    ############################################################################
    #                            END OF YOUR CODE                          
    ############################################################################
    return scores
...
class ResidualBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    if (Cin==Cout) and not downsample:
      ...
      self.shortcut = nn.Conv2d(Cin,Cout, 1,stride = 1)#1维卷积
    else:
